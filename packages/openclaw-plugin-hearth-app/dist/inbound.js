 ...
 */
async function downloadToWorkspace(url, ext) {
 ...
 */
async function stageImageAttachments(attachments) {
    const paths = [];
    const types = [];
...
export function createInboundHandler(account, cfg, channelRuntime) {
...
`;
            ...
            if (event.modelOverride)
                directives.push(`/model ${event.modelOverride}`);
            if (event.thinkLevel && event.thinkLevel !== 'off')
                directives.push(`/think ${event.thinkLevel}`);
            if (event.reasoningEnabled === true)
                directives.push(`/reasoning on`);
            ...
            await dispatchInboundReplyWithBase({
                ...
                replyOptions: {
                    onPartialReply: (payload) => {
                        const fullText = typeof payload === "string" ? payload : payload.text ?? "";
