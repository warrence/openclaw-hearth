...
function installCaddy() {
    ...
    try {
        if (platform === 'linux') {
            ...
            (0, child_process_1.execSync)('sudo apt-get install -y debian-keyring debian-archive-keyring apt-transport-https curl 2>/dev/null; ' +
                'curl -1sLf "https://dl.cloudsmith.io/public/caddy/stable/gpg.key" | sudo gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg 2>/dev/null; ' +
                'curl -1sLf "https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt" | sudo tee /etc/apt/sources.list.d/caddy-stable.list > /dev/null; ' +
                'sudo apt-get update -qq && sudo apt-get install -y caddy', { stdio: 'inherit', timeout: 120000 });
            return true;
        }
