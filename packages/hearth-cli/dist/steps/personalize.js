...
exports.runPersonalize = runPersonalize;
const inquirer_1 = __importDefault(require("inquirer"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const os = __importStar(require("os"));
async function runPersonalize(presetName) {
    console.log('');
    ...
    // Personality vibe
    const { vibe } = await inquirer_1.default.prompt([
        {
            type: 'list',
            name: 'vibe',
    ...
    // Communication style
    const { addressStyle } = await inquirer_1.default.prompt([
        {
            type: 'list',
            name: 'addressStyle',
    ...
    // Boundaries
    const { boundaries } = await inquirer_1.default.prompt([
        {
            type: 'checkbox',
            name: 'boundaries',
    ...
    const { special } = await inquirer_1.default.prompt([
        {
            type: 'input',
            name: 'special',
    ...
    // Household members
    const { addMembers } = await inquirer_1.default.prompt([
        {
            type: 'confirm',
            name: 'addMembers',
    ...
    if (addMembers) {
        let addMore = true;
        while (addMore) {
            const member = await inquirer_1.default.prompt([
                {
                    type: 'input',
                    name: 'name',
                    message: 'Member name:',
    ...
    try {
        const hearthConfig = fs.existsSync(hearthJsonPath)
